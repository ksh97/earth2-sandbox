from fastapi import FastAPI, HTTPException, Query
from fastapi.middleware.cors import CORSMiddleware

from earth2_sandbox.config import Settings, get_settings
from earth2_sandbox.services.forecast import (
    ForecastProviderUnavailable,
    ForecastSummary,
    MockForecastService,
    UnavailableForecastService,
)
from earth2_sandbox.services.fourcastnet import FourCastNetForecastService

LOCAL_DEV_ORIGINS = [
    "http://localhost:8081",
    "http://127.0.0.1:8081",
    "http://localhost:19006",
    "http://127.0.0.1:19006",
]


def create_app(settings: Settings | None = None) -> FastAPI:
    settings = settings or get_settings()
    app = FastAPI(
        title=settings.app_name,
        version="0.1.0",
        description="Backend API for the Earth-2 weather forecast sandbox.",
    )
    app.add_middleware(
        CORSMiddleware,
        allow_origins=LOCAL_DEV_ORIGINS,
        allow_credentials=True,
        allow_methods=["*"],
        allow_headers=["*"],
    )
    if settings.enable_mock_forecast:
        forecast_service = MockForecastService()
    else:
        try:
            forecast_service = FourCastNetForecastService.from_settings(settings)
        except ForecastProviderUnavailable as exc:
            forecast_service = UnavailableForecastService(str(exc))

    @app.get("/health")
    async def health() -> dict[str, str | bool]:
        return {
            "status": "ok",
            "service": settings.app_name,
            "environment": settings.environment,
            "mock_forecast": settings.enable_mock_forecast,
            "forecast_provider": "mock" if settings.enable_mock_forecast else "fourcastnet",
        }

    async def point_forecast_response(latitude: float, longitude: float) -> ForecastSummary:
        try:
            return await forecast_service.get_point_forecast(latitude=latitude, longitude=longitude)
        except ForecastProviderUnavailable as exc:
            raise HTTPException(status_code=503, detail=str(exc)) from exc

    @app.get("/api/v1/forecast/point", response_model=ForecastSummary)
    async def point_forecast(
        latitude: float = Query(..., ge=-90, le=90),
        longitude: float = Query(..., ge=-180, le=180),
    ) -> ForecastSummary:
        return await point_forecast_response(latitude=latitude, longitude=longitude)

    @app.get("/api/v1/forecast/sample", response_model=ForecastSummary)
    async def sample_forecast(
        latitude: float = Query(..., ge=-90, le=90),
        longitude: float = Query(..., ge=-180, le=180),
    ) -> ForecastSummary:
        return await point_forecast_response(latitude=latitude, longitude=longitude)

    return app
